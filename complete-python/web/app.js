...
const App = (() => {
    ...
    function jsonToCredentialForLogin(
        credential
    ) {
        /**
    ...
    async function registerAccount(
        ...
        try {
            ...
            const account =
                await MockAPI.registerAccount(
                    username,
                    password
                );

            showSuccess(
                `账户 ${account.username} 创建成功`
            );
            ...
            showError(
                (window.I18n ? window.I18n.error(error) : error.message)
            );
    ...
    async function loginAccount(
        ...
        try {
            ...
            const result =
                await MockAPI.loginAccount(
                    username,
                    password
                );

            showSuccess(
                `欢迎回来，${result.account.username}`
            );
    ...
    async function loginWithPasskey() {
        ...
        try {
            ...
            showSuccess(
                `登录成功，欢迎 ${result.account.username}`
            );
    ...
    async function init() {
        // 先绑定交互，避免 Session 请求失败后整个页面按钮失效。
        initRegisterPage();
        initLoginPage();
        try {
            await renderHeader();
...
document.addEventListener(
    "DOMContentLoaded",
    () => {
        App.init().catch(error => console.error("页面初始化失败", error));
    }
);
